              ...
              <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-for="cat in categories" :key="cat.id">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button @click="openModal(cat)" class="text-primary-600 hover:text-primary-900 dark:text-primary-400 mr-3">
                      Editar
                    </button>
                    <button @click="confirmDelete(cat)" class="text-red-600 hover:text-red-900 dark:text-red-400">
                      Excluir
                    </button>
                  </td>
                </tr>
...
const saveCategory = async () => {
  ...
  if (editingCategory.value) {
    result = await categoryStore.updateCategory(editingCategory.value.id, {
      name: form.name,
  ...
  if (result.success) {
    ...
    error.value = result.error || 'Erro ao salvar'
  }
}
...
const deleteCategory = async () => {
  ...
  if (result.success) {
    ...
    error.value = result.error || 'Erro ao excluir'
  }
}
</script>